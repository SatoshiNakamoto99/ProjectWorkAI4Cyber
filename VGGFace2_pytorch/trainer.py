...
class Trainer(object):
    ...
    def _init_(self, cmd, cuda, model, criterion, optimizer,
    ...
    def validate(self, make_inference=False):
        ...
        for batch_idx, (imgs, target, img_files, class_ids) in tqdm.tqdm(
            ...
            #print(predicted.item(),int(class_ids[0].split("n")[1]))
            # correct_predictions += (1 if predicted.item() == int(class_ids[0].split("n")[1]) else 0)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (imgs, target, img_files, class_ids) in tqdm.tqdm(
            ...
            if self.cuda:
                imgs, target = imgs.cuda(), target.cuda(True)
            imgs, target = Variable(imgs), Variable(target)
...
class Validator(Trainer):
    ...
    def __init__(self, cmd, cuda, model, criterion, val_loader, log_file, print_freq=1):
        super(Validator, self)._init_(cmd, cuda=cuda, model=model, criterion=criterion,
